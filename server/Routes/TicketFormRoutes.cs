...
public class TicketFormRoutes{
    ...
    public static async Task<TicketForm> GetTicketForm(int ticketId, NpgsqlDataSource db)
    {
        var result = new List<TicketForm>();
        await using var command = db.CreateCommand("SELECT id, company_id, title, description FROM tickets WHERE id = $1");
